...
namespace Mitty.Osu
{
    class DifficultyGraph : DataGraph
    {
        ...
        public DifficultyGraph(Input input, BeatmapData beatmap)
        {
            ...
            var background = Image.FromStream(backgroundStream);
            ...
            plt.PlotScatter(dataX, difficulty.StrainPeakTotal, markerShape: MarkerShape.none, label: "Strain", lineWidth: 3, color: Color.DodgerBlue);
            plt.PlotScatter(dataX, difficulty.StrainPeakAim, markerShape: MarkerShape.none, label: "Aim", lineWidth: 1, color: Color.MediumAquamarine);
